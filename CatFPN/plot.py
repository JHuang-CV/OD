...
def single_gpu_test(model, data_loader, show=False):
    ...
    for i, data in enumerate(data_loader):
        ...
        with torch.no_grad():
            feature_maps = model(return_loss=False, rescale=not show, **data)

        batch_size = data['img'][0].size(0)
        for b in range(batch_size):
            prog_bar.update()
            filename = data['img_meta'][0]._data[0][0]['filename']
            picture = filename.split('/')[-1].split('.')[0]
            if not os.path.exists(f'./feature_maps/{picture}'):
                os.makedirs(f'./feature_maps/{picture}')

            img = cv2.imread(filename)
            # cv2.imwrite(f'./feature_maps/{picture}/{picture}.jpg', img)

            for n in range(len(feature_maps)):

                heatmap = feature_maps[n][b]
                ...
                cv2.imwrite(f'./feature_maps/{picture}/heat_{b}_{n}.jpg', heat_img)
