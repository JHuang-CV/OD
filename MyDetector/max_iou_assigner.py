...
class MaxIoUAssigner(BaseAssigner):
    ...
    def assign_wrt_overlaps(self, overlaps, gt_labels=None):
        ...
        # new add
        bbox_gt_IoU = overlaps.new_zeros((num_bboxes,))
        pos_inds = torch.nonzero(assigned_gt_inds > 0).squeeze()
        if pos_inds.numel() > 0:
            bbox_gt_IoU[pos_inds] = overlaps[assigned_gt_inds[pos_inds] - 1, pos_inds]

        return AssignResult(
            num_gts, assigned_gt_inds, max_overlaps, bbox_gt_IoU, soft_gt_labels,
            unassigned_bboxes_inds, labels=assigned_labels)


class AssignResult(object):

    def __init__(self, num_gts, gt_inds, max_overlaps, bbox_gt_IoU,
                 soft_gt_labels, unassigned_bboxes_inds, labels=None):
        self.num_gts = num_gts
        self.gt_inds = gt_inds
        self.max_overlaps = max_overlaps
        self.bbox_gt_IoU = bbox_gt_IoU
        self.soft_gt_labels = soft_gt_labels
        self.unassigned_bboxes_inds = unassigned_bboxes_inds
        self.labels = labels
